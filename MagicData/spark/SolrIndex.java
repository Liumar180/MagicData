...
public class SolrIndex implements IndexProvider {
                ...
                for (Map.Entry<String, IndexMutation> entry : stores.getValue().entrySet()) {
                    ...
                    if (mutation.hasAdditions()) {
                        ...
                        for (IndexEntry e : mutation.getAdditions()) {
                            ...
                            // xqw modified at 2016-04-14
                            //if("LIST".equals(e.getMetaData().get("Cardinality")))
                            //final String fieldName = mapKey2Field("phonenumberlist", informations.get(entry.getValue()., collectionName));
                            //if("phonenumberlist".equals(e.field))
                            if(isCardinalityList(informations.get(collectionName,e.field)))
                            	doc.addField(e.field, isNewDoc
    ...
    // xqw added at 2016-04-15
    private boolean isCardinalityList(KeyInformation keyInfomation) {
    	if(keyInfomation == null)
    		return false;
    	
    	for(Parameter p : keyInfomation.getParameters()) {
    		if("Cardinality".equals(p.getKey()) && "LIST".equals(p.getValue()))
    			return true;
    	}
