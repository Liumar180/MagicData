...
public class modifyGroupData {

	public static void main(String[] args) {
		TitanGraph graph = TitanConnection.getTitaConnection();
		int r = 100000;
		List<Map<String,String>> ss = null;
		//for(int k=100000; k<=3600000; k +=r){
		if(true){
			ss = modifyGroupData.getGrouplistDatas(3600000,1188753);
		long st = System.currentTimeMillis();
		String startTime = DateFormat.transferLongToDate(st);
		System.out.println("开始时间：" + startTime);
		WritelogContents.writeLogs("C:\\Users\\integrity\\Desktop\\solr\\deletePerson.txt", "开始时间："+startTime);
		for(int i=0;i<ss.size();i++){
			modifyGroupData.deleGroupsP(ss.get(i), graph);
		}
		long et = System.currentTimeMillis();
		String endTime = DateFormat.transferLongToDate(et);
		System.out.println("结束时间：" + endTime);
		WritelogContents.writeLogs("C:\\Users\\integrity\\Desktop\\solr\\deletePerson.txt", "结束时间："+endTime);
		WritelogContents.writeLogs("C:\\Users\\integrity\\Desktop\\solr\\deletePerson.txt", "本次删除数据量："+ss.size()+"个");
		WritelogContents.writeLogs("C:\\Users\\integrity\\Desktop\\solr\\deletePerson.txt", "总用时："+(et-st)/1000+"秒");
		}
		graph.shutdown();
	...
	public static void QQgroups(Map<String,String> maps,TitanGraph graph){
		...
		WritelogContents.writeLogs("C:\\Users\\integrity\\Desktop\\solr\\ded.txt", "数据numid="+numid);
	       ...
	       if(s != null){
	    	  ...
	    	  if(iis){
	    		  ...
	    		  try {
					...
					WritelogContents.writeLogs("C:\\Users\\integrity\\Desktop\\solr\\ded.txt", "没有人物对象"+numid);
	 ...
	 * 删除添加重复的人
	 */
	public static void deleGroupsP(Map<String,String> maps,TitanGraph graph){
		 String numid = maps.get("numid");
		 WritelogContents.writeLogs("C:\\Users\\integrity\\Desktop\\solr\\deletePerson.txt", "数据numid="+numid);
	...
	public static List<Map<String,String>> getGrouplistDatas(Integer counts,Integer rows) {
		WritelogContents.writeLogs("C:\\Users\\integrity\\Desktop\\solr\\deletePerson.txt", "查询总记录数："+counts+"开始查询行数："+rows+"区间-》"+rows+"---"+counts);
    ...
    String sql = "SELECT c.MEMBERNUM FROM ( SELECT s.MEMBERNUM,ROWNUM RN "
				+" FROM (SELECT  DISTINCT(g.MEMBERNUM)"
				+" from TB_RNS_QQ_GROUPLIST g "
